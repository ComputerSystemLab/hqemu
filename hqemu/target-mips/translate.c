                                ...
                                int search_pc)
{
    ...
    if (env->gen_trace == 1 && tb->size != ctx.pc - pc_start) {
        /* consistency check with tb info. we must make sure 
	   guest basic blocks are the same. skip this trace if inconsistant */
        fprintf(stderr, "inconsistant block with pc 0x%lx %d (%d)\n", 
			(unsigned long)tb->pc, tb->size, ctx.pc - pc_start);
        exit(0);
