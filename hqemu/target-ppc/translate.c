                                                  ...
                                                  int search_pc)
{
    ...
    if (env->gen_trace == 1 && tb->size != ctx.nip - pc_start) {
        ...
        fprintf(stderr, "inconsistant block with pc 0x%lx size %d (%d)\n",
                        (unsigned long)tb->pc, tb->size, ctx.nip - pc_start);
        exit(0);
    ...
    if (ctx.exception == POWERPC_EXCP_NONE) {
        ...
        gen_ibtc_stub(ctxp, tcg_const_ptr(ctx.nip));
        tb->jmp_pc[0] = ctx.nip;
#endif
