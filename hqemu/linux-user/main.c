...
static inline void copy_tcg_context(CPUState *env)
{
    tcg_ctx_env = (TCGContext *)qemu_malloc(sizeof(TCGContext));
    memcpy(tcg_ctx_env, &tcg_ctx, sizeof(TCGContext));
    tcg_ctx_env->temps = tcg_ctx_env->static_temps;
    
    env->gen_opc_buf = (uint16_t *)qemu_malloc(OPC_BUF_SIZE * sizeof(uint16_t));
    env->gen_opparam_buf = (TCGArg *)qemu_malloc(OPPARAM_BUF_SIZE * sizeof(TCGArg));
    env->gen_opc_instr_start = (uint8_t *)qemu_malloc(OPC_BUF_SIZE * sizeof(uint8_t));
}
