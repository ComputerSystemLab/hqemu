...
static TCGv cpu_cc_src, cpu_cc_dst, cpu_cc_tmp;
static TCGv_i32 cpu_cc_op;
static TCGv cpu_regs[CPU_NB_REGS];
/* local temps */
static __thread TCGv cpu_T[2], cpu_T3;
...
#ifdef ENABLE_IBCHAIN
	*gen_opc_ptr++ = INDEX_op_goto_ibchain;
	*gen_opparam_ptr++ = ibtc_guest_eip;
	*gen_opparam_ptr++ = (tcg_target_long)(long)s->tb;
#else
...
static void gen_sse(DisasContext *s, int b, target_ulong pc_start, int rex_r)
{
    ...
    if (!is_xmm) {
        ...
        if (s->env->gen_trace == 1)
            s->env->gen_trace_skip = 1;
    }
                                                  ...
                                                  int search_pc)
{
    ...
    for(;;) {
        ...
        if (env->gen_trace == 1 && num_insns == tb->icount) {
            gen_jmp_im(pc_ptr - dc->cs_base);
            gen_eob(dc);
            break;
        }
...
#if defined(CONFIG_HYBRID)
            gen_ibtc_stub(dc, cpu_tmp0);
            tb->jmp_pc[0] = pc_ptr - dc->cs_base;
#endif
            gen_eob(dc);
            break;
        }
    ...
    if (env->gen_trace == 1 && tb->size != pc_ptr - pc_start) {
        /* consistency check with tb info. we must make sure 
	   guest basic blocks are the same. skip this trace if inconsistant */
        fprintf(stderr, "inconsistant block with pc 0x%lx\n", (unsigned long)tb->pc);
        exit(0);

        env->gen_trace_skip = 1;
        return;
    }
