...
extern FILE *logfile;
extern int loglevel;
extern pthread_mutex_t qemu_log_lock;
...
#define qemu_log(...) do {                 \
        if (logfile) {                     \
            pthread_mutex_lock(&qemu_log_lock);   \
            fprintf(logfile, ## __VA_ARGS__); \
            pthread_mutex_unlock(&qemu_log_lock); \
	} \
    } while (0)
...
#define log_target_disas(start, len, flags) do { \
        pthread_mutex_lock(&qemu_log_lock);   \
        target_disas(logfile, (start), (len), (flags)); \
        pthread_mutex_unlock(&qemu_log_lock); \
    } while (0)
#define log_disas(start, len) do { \
        pthread_mutex_lock(&qemu_log_lock);   \
